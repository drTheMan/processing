...
class TunnelApp < Processing::App
  ...
  class Swirly
    ...
    def current_color
      @current_color ||= color(0, 0, 0) #random_color
    ...
    def randomize_color?
      @randomize_color.nil? ? @randomize_color = (options[:randomize_color] == true) : @randomize_color
    end
    ...
    def draw_layer(layer)
      pushMatrix
        translate layer.x || x, layer.y || y
        rotate layer.rotation

        noStroke
        # stroke(color(255,255,255))
        fill color(layer.color)
        rect -(layer.w*0.5), -(layer.h*0.5), layer.w, layer.h
      popMatrix
    end
    ...
    def generate_layer
      ...
      Layer.new(:rotation => generate_target_rotation, :color => get_color, :x => x + random(30), :y => y+random(30), :w => w, :h => h, :gap => get_gap)
    end
    ...
    def generate_target_rotation
      ...
      distance = 0.05 * PI
      random(-distance, distance) # + rotation
    end
